...
def estimation_by_point_mp(IDHOLEs, out_q, distancia, ker):
    n = len(IDHOLEs)
    dicc_preds = {}
...
def mp_gaussian_process_by_test_point(IDHOLEs, nprocs, ker, distancia=30):
...
if __name__ == '__main__':
    ...
    dist = 20
    t0 = time.time()
    diccionario = mp_gaussian_process_by_test_point(HOLEIDs, 7, kernel, distancia=dist)
    print('Tiempo para gp con multiprocessing: {}'.format(time.time()-t0))

    # exportar los datos
    outfile_name = 'mp_gpflow_'+kernel.name+'_'+str(dist)+'.csv'
    path_estimacion = 'estimaciones/'
    outfile = open(path_estimacion + outfile_name, 'w')
    outfile.write('xcentre,ycentre,zcentre,minty,cut_poz,cut,f1,muestras\n')
    for holeid in HOLEIDs:
